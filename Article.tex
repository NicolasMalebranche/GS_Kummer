\documentclass[a4paper]{article}

\input{HeaderFile.tex}

\begin{document}

\title{\bf Integral cohomology of the Generalized Kummer fourfold}


\author{Simon Kapfer and Gr\'egoire Menet}
%\address{Simon Kapfer, Institut f\"ur Mathematik, Universit\"at Augsburg, D--86159 Augsburg}
%\email{simon.kapfer@math.univ-poitiers.fr}
%\email{simon.kapfer@math.uni-augsburg.de}
%\urladdr{http://www.math.uni-augsburg.de/alg/}


\date{\today}

\maketitle
\begin{abstract}
We give a description of integral cohomology of the Generalized Kummer fourfold, using Hilbert scheme cohomology and Nakajima operators.
Then we apply our results to a IHS manifold with singularities, obtained by quotienting the Generalized Kummer by an involution. 
We calculate the Beauville--Bogomolov form of this new manifold, giving the first example of such a form that is odd.
\end{abstract}

\section{Introduction}
\part{Preliminaries}
\input{Section_Lattices.tex}
\input{Section_Exterior.tex}
\input{Section_Symplectic.tex}
\input{Section_Abelian.tex}
\input{Section_Recall_Integral}
\input{Section_OddH2.tex}
\input{Section_Hilbert.tex}
\part{The Generalized Kummer fourfold}\label{integralcohomology}
\input{Section_GeneralKummer.tex}
\input{Section_Kummer4.tex}
\input{Section_H4.tex}
\input{SummaryKummerFourfold.tex}
\part{A quotient}\label{quotient}
%\section{Symplectic involution on a $\kum{A}{2}$-type ISM}
%\section{$H^4$-normality and $H^2$-normality}
%\section{On $H^2(K,\Z)$}
%\section{Calculation of the Beauville-Bogomolov form}
\input{ApplicationKummer}

\input{Bibliography.tex}

\end{document}