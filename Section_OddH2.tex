...
Let $A$ be a smooth compact surface with torsion free cohomology and $A\hilb{2}$ the Hilbert scheme of 2 points. 
...
, so we have $j: E\rightarrow \widetilde{A\times A}  $. 
...
By (\ref{classicexact}) of the last section, there is an exact sequence
$$
...
\end{proposition}
The section is dedicated to the proof of this proposition. The proof is organized as follows. Section 7.1 is devoted to calculate the torsion of $H^{3}(A\hilb{2}\smallsetminus E,\Z)$ (Lemma \ref{3}) using equivariant cohomology technique developed in \cite{Lol}. Then this knowledge allow us to deduce $\alpha_3=0$ using the exact sequence (\ref{exactutile}) and $\alpha_5=4$ using the unimodularity of the lattice $H^{3}(\widetilde{A\times A},\Z)\oplus H^{5}(\widetilde{A\times A},\Z)$.
...
\label{ThomII}
\end{equation}
By proof of Theorem 7.31 of \cite{Voisin}, the map $h$ is injective and its image in $H^{3}(\widetilde{A\times A},\Z)$ is $j_*(b_{|E}^{*}(H^{1}(\Delta,\Z)))$. Hence Diagram (\ref{ThomII}) shows that $g$ is also injective and has image $\pi_{*}(j_*(b_{|E}^{*}(H^{1}(\Delta,\Z))))$ in $H^{3}(A^{[2]},\Z)$.
...
\end{proof}
It prove (i) of Proposition \ref{Alpha35}.
\subsection{In degree 5}\label{d5}
...
$$\frac{\pi_*(H^3(\widetilde{A\times A},\Z)\oplus H^5(\widetilde{A\times A},\Z))}{\pi_*(H^3(\widetilde{A\times A},\Z)^\iota\oplus H^5(\widetilde{A\times A},\Z)^\iota)}=(\Z/2\Z)^{l_2^3(\widetilde{A\times A})+l_2^5(\widetilde{A\times A})}.$$
It follows:
