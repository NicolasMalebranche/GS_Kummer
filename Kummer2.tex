...
\begin{equation}
\q_1(b_1)\cdots \q_1(b_n)\vac = \rho^*\left( \sum_{\pi\in\mathfrak{S}_n } b_{\pi(1)}\otimes\ldots\otimes b_{\pi(n)} \right), \quad b_i\in H^*(A,\Q).
\end{equation}
In particular,
...
\end{gather}
This is sometimes useful for manually computing products. For instance, if $b\in H^2(A,\Q)$ is of degree $2$, then
\begin{equation}\label{qprimitivepower}\begin{array}{rl}
\left(\frac{1}{(n-1)!}\q_1(b)\left(\q_1(1)\right)^{n-1}\vac\right)^{2n}
...
\end{equation}
Combining this formula with (\ref{fujiki}) and (\ref{q1primitive}), we get
$\int_A b^2 = q(\alpha)$.

Secondly, we must show that $q(\theta^*\delta) = -2n$. TODO: show this! 
