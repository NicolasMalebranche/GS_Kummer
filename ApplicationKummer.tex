...
Hence we do not lose any generality assuming that $\iota=-\id_A$. Now, we calculate the invariants $l_i^j(K_{2}(A))$ define in Definition-Proposition \ref{defiprop}. It will used in Section \ref{BeauvilleForm}.
...
\end{lemme}
\begin{proof}
We know from Remark \ref{SPA2} that the monodromy representation on $A[2]$ contains the symplectic group $\Sp A[2]$. 
We recall from Remark \ref{RemarkSymplecticInv} (2), that the $D_1,...,D_{35}$ are given by $\pi_2(s^{-1}(\mathcal{P})$.
It follows that the monodromy representation on $H^2(\widetilde{K},\Z)$ contains the isometries which act on $D_1,...,D_{35}$
