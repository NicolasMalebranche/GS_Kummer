...
\newtheorem{contr}[thm]{Counter-example}
...
\begin{document}
\title{\bf Odd cohomology of $A^{[2]}$}

\author{Grégoire \textsc{Menet}} 

\maketitle

With your notation, we consider the following exact sequence:
$$\xymatrix{ 0\ar[r]&\pi_{*}(H^{k}(\widetilde{A\times A},\Z))\ar[r] & H^{k}(A^{[2]},\Z)/\tors\ar[r] & (\Z/2\Z)^{\alpha_{k}}\ar[r]& 0,}$$
with $k\in \left\{1,...,8\right\}$.
We want to prove the following proposition.
\begin{prop}
We have:
$$\alpha_3=0\ \et\ \alpha_5=4.$$
\end{prop}
\section{Preliminary Lemmas}
%Fist we need to calculate the following invariant:
We denote $V=\widetilde{A\times A}\smallsetminus E$ and $U=V/\sigma_{2}$, where $\mathfrak{S}_{2}=\left\langle \sigma_{2}\right\rangle$. 
\begin{lemme}\label{1}
...
\end{proof}
Now we will calculate the invariant $l_{1,-}^{2}(A\times A)$ and $l_{1,+}^{1}(A\times A)$ defined in Section 1.2 of \cite{Lol} (I also recall the definition in the redaction of the application).
\begin{lemme}\label{2}
...
By Künneth formula we also have:
\begin{align*}
...
\end{lemme}
\begin{proof}
Using the spectral sequence of equivariant cohomology, it follows from Proposition 2.6 of \cite{Lol}, Lemma \ref{1} and \ref{2}.
\end{proof}
\section{Proof of $\alpha_3=0$}\label{=}
...
and
\begin{equation}
l_{1,+}^3(\widetilde{A\times A})=\rk H^{1}(\Delta,\Z)^{\mathfrak{S}_2}=4,\ \et\ l_{1,-}^3(\widetilde{A\times A})=0.
\label{l4}
\end{equation}
...
and 
\begin{equation}
l_{1,+}^5(\widetilde{A\times A})=\rk H^{3}(\Delta,\Z)^{\mathfrak{S}_2}=4,\ \et\ l_{1,-}^5(\widetilde{A\times A})=0.
\label{l3}
\end{equation}
\begin{lemme}
...
\end{lemme}
\begin{proof}
By Definition-Proposition 1.7 2) and 3) of \cite{Lol}, (\ref{l1}) and (\ref{l2}):
...
Then by Lemma 2.18 3) of \cite{Lol},
...
Then by Proposition 1.6 of \cite{Lol}:
...
Then by Lemma 2.17 and 2.3 of \cite{Lol}, 
...
However, by Section \ref{=}, we know that $\pi_{*}(H^{3}(\widetilde{A\times A},\Z))=H^{3}(A^{[2]},\Z)$.
It follows 
...
\bibitem{Lol}
G.~Menet
\newblock {\em On the integer cohomology of quotients of Kähler manifolds},
\newblock arXiv:1312.1584v3[math.AG] 4 Mar 2015.
