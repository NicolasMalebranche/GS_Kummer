...
\end{thm}
\begin{rmk}
Because of the Poincaré duality and the universal coefficient theorem, we only have to prove that $H^3(K_2(A),\Z)$ and $H^5(K_2(A),\Z)$ are torsion free. Moreover, since Theorem \ref{main} is only a topological result, without loss of generality, we can assume that $A$ is an abelian surface.
\end{rmk}
